...
sub dump_th {
    ...
    my @Content;
    push (@Content, "", "Rider", "ChipID", "Chip");
...
sub dump_tr {
    my ($cgi, $racetype, $count, $lapcurrent, $maxlapsdown, $fullname, $chipid, $chip, $mylapsdown, $start, $end, $points, $lapped, @laptimes) = @_;
    ...
    push (@Content, $cgi->td($chipid));
    push (@Content, $cgi->td($chip));
    ...
    if ($racetype eq "Points Race") {
            ...
            foreach ( sort {$$b{'points'} <=> $$a{'points'} || $$a{'finish'} <=> $$b{'finish'}} @results) {
                ...
                push(@Content, 
                        $cgi->Tr({ -class => $tr_class, -align => "CENTER", -valign => "BOTTOM" }, 
                            dump_tr($cgi, $racetype, $count, $LapCurrent, 0, $key, $ChipID{$key}, $Chip{$key}, 0, $First, $last, $Points{$key}, $lapped{$key} && $final, @{$behind_ref})));
        ...
        for (my $m = 0; $m < 2; $m++ ) {
            foreach ( sort {$$a{'downlaps'} <=> $$b{'downlaps'} || $$a{'finish'} <=> $$b{'finish'}} @results) {
                ...
                push(@Content, 
                        $cgi->Tr({ -class => $tr_class, -align => "CENTER", -valign => "BOTTOM" }, 
                            dump_tr($cgi, $racetype, $count, $LapCurrent, 0, $key, $ChipID{$key}, $Chip{$key}, $downlaps, $First, $last, 0, $lapped{$key} && $final, @{$behind_ref})));
                $count++;
            }
        }
    }
...
sub do_one {
    ...
    #printf STDERR "workoutlap: %d maxlaps: %d LapCorrection: %d\n", $workoutlap, $maxlaps, $LapCorrection;
    next if ($maxlaps && $workoutlap > ($maxlaps + $LapCorrection));
    ...
    for (my $table = 0; ; $table++) {
        ...
        for (my $m = 0; $m <= $MaxLapsDown + 1; $m++) {
            ...
            foreach my $key (sort { $laporder{$a} <=> $laporder{$b}} keys %laporder)  {
                ...
                push(@Content, 
                        $cgi->Tr({ -class => $tr_class, -align => "CENTER", -valign => "BOTTOM" }, 
                            dump_tr($cgi, $racetype, $count, $lapcurrent, $MaxLapsDown, $key, $ChipID{$key}, $Chip{$key}, $m, $first, $last, $points{$key}, 0, @{$behind_ref})));
